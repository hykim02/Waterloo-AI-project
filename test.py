...
model = torchvision.models.detection.ssdlite320_mobilenet_v3_large(num_classes=len(categories) + 1,
trainable_backbone_layers=6)
model.load_state_dict(torch.load('./models/model_epoch_20.pth'))
model.to(device)
model.eval()
